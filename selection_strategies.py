...
def selection_with_class_centeroids(
    ...
    for label_idx in id2label.keys():
        ...
        tuples = [(i, l, d) for (i, l), d in zip(class_labels, dists)]
        closest_to_center = sorted(tuples, key=lambda x: x[2], reverse=True if selection_strategy == "closest" else False)
        selected_examples.extend([idx for idx, _, _ in closest_to_center[:num_examples_per_class]])
...
def expected_gradients_selection(
    ...
    if f"{dataset}-expected-gradients.json" in os.listdir(".cache/"):
        expected_gradients = json.load(".cache/f{dataset}-expected-gradients.json")
        ...
        batch_size = 16
        if len(text_column) == 1:
            loader = DataLoader(dataset["train"][text_column[0]], batch_size=batch_size, shuffle=False)
        else:
            raise NotImplementedError
        ...
        with open(f".cache/f{dataset}-expected-gradients.json") as f:
